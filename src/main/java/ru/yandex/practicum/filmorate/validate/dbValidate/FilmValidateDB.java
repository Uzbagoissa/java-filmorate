...
public class FilmValidateDB {
    ...
    public void checkAddLikeValidate(Logger log, SqlRowSet userRows, Integer userId, Integer id) {
        if(userRows.next()) {
            log.error("Пользователь {} уже поставил свой лайк фильму {}!", userId, id);
            throw new NotFoundException("У этого фильма уже есть лайк от этого пользователя!");
        }
    }
